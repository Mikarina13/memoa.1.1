...
import { Heart, MapPin, Film, BookOpen, Quote, Globe, Plus, Trash2, Save, Upload, Download, ExternalLink, CheckCircle, Gamepad2 } from 'lucide-react';
...
interface PersonalPreferencesInterfaceProps {
  ...
  onClose?: () => void;
  context?: 'memoir' | 'memoria'; // Add context to distinguish between MEMOIR and MEMORIA
...
export function PersonalPreferencesInterface({ onPreferencesSaved, onClose, context = 'memoir', initialTab = 'favorites' }: PersonalPreferencesInterfaceProps) {
  const { user } = useAuth();
  ...
  }, [preferences, hasUnsavedChanges, autoSaveEnabled, user, context]);

  const loadExistingPreferences = async () => {
    try {
      setIsLoading(true);
      console.log(`Loading ${context} personal preferences for user:`, user.id);
      
      const personalData = await MemoirIntegrations.getPersonalPreferences(user.id, context);
      ...
      console.error(`Error loading ${context} personal preferences:`, error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSavePreferences = async (isAutoSave: boolean = false) => {
    ...
    try {
      ...
      const personalData = await MemoirIntegrations.storePersonalPreferences(user.id, preferences, context);
  ...
  const exportPreferences = () => {
    ...
    a.href = url;
    a.download = `${context}-personal-preferences-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
  ...
  const title = context === 'memoria' ? 'Personal Favorites of Your Loved One' : 'Personal Preferences';
      ...
      <div className="bg-black/90 border border-white/20 rounded-xl p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-4 p-3 bg-white/5 rounded-lg border border-white/10">
          <div className="flex items-center gap-2">
            <div className={`w-3 h-3 rounded-full ${context === 'memoria' ? 'bg-purple-400' : 'bg-blue-400'}`}></div>
            <span className="text-white/80 text-sm font-medium">
              {context === 'memoria' ? 'MEMORIA Mode' : 'MEMOIR Mode'}
            </span>
            <span className="text-white/60 text-sm">
              - {context === 'memoria' ? 'Preserving your loved one\'s favorites' : 'Recording your personal preferences'}
            </span>
          </div>
        </div>
          ...
          <div className="bg-white/5 rounded-lg p-6">
            ...
            <div className="p-4 bg-pink-500/10 border border-pink-500/20 rounded-lg">
              <h4 className="text-pink-400 font-medium mb-2">
                {context === 'memoria' ? 'Memorial Legacy Benefits:' : 'Legacy Benefits:'}
              </h4>
              <ul className="text-white/70 text-sm space-y-1">
                {context === 'memoria' ? (
                  <>
                    <li>• Preserve their favorite entertainment and inspirations</li>
                  ...
                  <>
                    <li>• Share your favorite entertainment with loved ones</li>
