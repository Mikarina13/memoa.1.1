...
export function GamingPreferencesInterface({ onGamesStored, onClose }: GamingPreferencesInterfaceProps) {
  const { user } = useAuth();
  ...
  const loadExistingGames = async () => {
    try {
      const gamingData = await MemoirIntegrations.getGamingPreferences(user.id);
      if (gamingData?.games) {
  ...
  const handleSaveGames = async () => {
    ...
    try {
      const gamingData = await MemoirIntegrations.storeGamingPreferences(user.id, games);
      ...
      <div className="bg-black/90 border border-white/20 rounded-xl p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-bold text-white font-[Orbitron]">Gaming Preferences</h2>
          </div>
          <button
            onClick={onClose}
